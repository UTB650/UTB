...
function validateWorkout(workout: any, userProfile: any): boolean {
  ...
  const sectionNames = workout.sections.map((s: any) => s.section_name);
  ...
  const totalDuration = workout.sections.reduce((sum: number, section: any) => sum + section.duration_minutes, 0);
